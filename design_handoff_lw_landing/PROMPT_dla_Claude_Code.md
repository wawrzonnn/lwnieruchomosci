...
- `RegionGallery.svelte` (pozioma galeria — wheel/drag/strzałki/progress, patrz „Kluczowe elementy")
- `Testimonials.svelte` (rotujący cytat + kropki, patrz „Kluczowe elementy")
- `DualCta.svelte` (przekątny split), `Contact.svelte`, `Footer.svelte`
...
4. **Galeria Regionu — pozioma, przewijana na 3 sposoby (NIE automatyczna, NIE scroll-jacking):**
   - Kółko myszy nad galerią przewija ją w BOK (mapuj `deltaY` pionowego scrolla na `scrollLeft`, z `preventDefault()`).
   - Przeciąganie kursorem (mousedown/mousemove/mouseup na `window`, nie tylko na elemencie) — wyłącz `scroll-snap-type` na czas przeciągania.
   - Dwie okrągłe strzałki `‹ ›` wołające `scrollBy({left: ±580, behavior:'smooth'})`.
   - Cienki złoty pasek postępu pod galerią, aktualizowany na evencie `scroll` KONTENERA (nie strony).
   - **Absolutnie nie implementuj tego jako `position:sticky` z przypięciem na scrollu całej strony** — próbowaliśmy, psuje się w statycznych/paginowanych podglądach (puste białe/ciemne bloki). Zwykły `overflow-x:auto` jest poprawny i wystarczający.
5. **Opinie — pojedynczy rotujący cytat** na zdjęciu w tle, auto-rotacja co 6,5s + klikalne kropki (aktywna = wydłużona pigułka `#E9D19A`). NIE siatka 3 kart.
6. **Dual CTA — przekątny podział** pełnej szerokości (`clip-path: polygon(...)` na obu panelach, dopasowane kąty tworzą jedną ciągłą ukośną linię), z wyśrodkowaną obróconą odznaką „Wybierz swoją ścieżkę" na przecięciu. NIE dwie oddzielne zaokrąglone karty obok siebie.
...
- Nadtytuły sekcji („eyebrow"): 12px, uppercase, `letter-spacing:.16em`, `#B4894C` (na zielonym/ciemnym tle: `#D9BE84`).
- Promienie: pill 999px (przyciski/badge/chipy), karty 16–20px, pola/przyciski 12px.
- Kolejność sekcji: Nav → Hero(+wyszukiwarka) → Trust → Kategorie → Oferty → O nas → Usługi → **Region (galeria pozioma)** → **Opinie (rotujący cytat)** → **Dual CTA (przekątna)** → Kontakt+mapa → Stopka.
