						...
						{#if formError}<p style="color:#b3261e;font-size:14px;margin-top:6px;">{formError}</p>{/if}
							...
							<label class="lead-field">
								...
								<textarea
									name="message"
									rows="3"
									placeholder="Napisz kilka słów o nieruchomości i celu promocji…"
								></textarea>
							</label>
							<button type="submit" class="lead-submit">{formularz.przycisk}</button>
							<p class="lead-fineprint">
								Wysyłając formularz, zgadzasz się na kontakt w sprawie promocji. Nie wysyłamy spamu.
							</p>
							<p class="lead-rodo">
...
<style lang="scss">
	...
	@media (max-width: 980px) {
		...
		/* Gradient z markupu biegnie po skosie w lewo — działa, dopóki tekst
		   siedzi w lewej kolumnie. Na telefonie akapit zajmuje całą szerokość
		   i środek zdań wypadał na jasnym niebie. Tło jest ustawiane inline,
		   więc przyciemnienie dokładamy pseudoelementem. */
		.hero-svc::before {
			content: '';
			position: absolute;
			inset: 0;
